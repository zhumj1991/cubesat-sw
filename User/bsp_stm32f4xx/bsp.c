...
void BSP_Init(void)
{
	...
	spi_flash_read_buffer((uint8_t *)&obc_status, CPU_STATUS_BASE, sizeof(obc_status));
	OSSemPost (&SEM_SPI1_DEV_MUTEX, OS_OPT_POST_1, &err);

	if(obc_status.flag == 0xff)
		memset(&obc_status, 0, sizeof(obc_status));
	else {
		obc_status.power_on++;
		debug("CPU reset %d times\r\n", obc_status.power_on);
		rtc_set(&obc_status.tm);
	}
...
*********************************************************************************************************
*/
void BSP_DelayUS(uint32_t time_us)
{
#if 0
	uint32_t ticks;
	uint32_t told,tnow,tcnt=0;
		    	 
	ticks = time_us * 168;      /* 需要的节拍数 */	  		 
	tcnt = 0;
